...
# file locations
df_values_location = "values.csv"
df_labels_location = "labels.csv"
...
# load dataset  # error formatting properly  # this is the weirdest goddamn format for processed_data i've ever seen. why does everything need to be double nested???
data_values = pd.read_csv(f"processed_data/{df_values_location}").tolist()
...
data_labels = np.array(pd.read_csv(f"processed_data/{df_labels_location}")).tolist()
...
if graphs:
    # find label names
    label_names = pd.read_csv(f"processed_data/{df_labels_location}", nrows=0).columns.tolist()
    # graph cms
