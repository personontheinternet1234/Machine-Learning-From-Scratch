...
# load dataset
df_values = np.array(pd.read_csv(f"assets/processed_data/{df_values_location}")).tolist()
for i in tqdm(range(len(df_values)), ncols=150, desc="Reformatting Data Values"):
    df_values[i] = np.array([df_values[i]])
df_labels = np.array(pd.read_csv(f"assets/processed_data/{df_labels_location}")).tolist()
...
if graphs:
    # find label names
    label_names = pd.read_csv(f"assets/processed_data/{df_labels_location}", nrows=0).columns.tolist()
    # graph cms
