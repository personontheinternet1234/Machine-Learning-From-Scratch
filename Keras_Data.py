...
data_values = []
data_labels = []
for i in tqdm(range(len(train_values)), ncols=100, desc="Reformatting Data"):
...
# generate data names
name_values = []
for i in tqdm(range(len(train_values[0]) ** 2), ncols=100, desc="Generating Value Names"):
    name_values.append(i)
name_labels = []
for i in tqdm(range(num_labels), ncols=100, desc="Generating Label Names"):
    name_labels.append(i)

# save keras data
with open(save_location_values, "w") as f:
    f.write(str(name_values).strip("[").strip("]") + "\n")
    for array in tqdm(range(len(data_values)), ncols=100, desc="Saving Values"):
...
with open(save_location_labels, "w") as f:
    ...
    for array in tqdm(range(len(data_labels)), ncols=100, desc="Saving Labels"):
