...
"""
This program uses the nodes structure to practice basic backpropagation, adjusting a final bias to fit a curve to a set of processed_data points.
...
def new_value(actualDataSet, oldWeight):  # gradient descent function for a given connection's weight.
    ...
    # sum of each processed_data point: 2 * (observed - predicted) * -1
...
def train(i):
    ...
    if what_epoch < epochs:
        ...
        # plotting machine's processed_data set x and y
        predictedx = []
        predictedy = []
        for i in range(100):
        ...
        # plotting actual processed_data set x and y
